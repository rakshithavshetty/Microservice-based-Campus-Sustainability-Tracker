...
const WaterConsumptionChart = () => {
  const { waterConsumption, loading, error } = useReportContext();
  ...
  useEffect(() => {
    setMlLoading(true);
    setMlError(null);
    reportApi.fetchWaterConsumptionTrend()
  ...
  // Group data by year
  const yearMap = {};
  waterConsumption.forEach((d) => {
    const [mm, yyyy] = d.month.split('-');
    if (!yearMap[yyyy]) yearMap[yyyy] = Array(12).fill(null);
    yearMap[yyyy][parseInt(mm, 10) - 1] = d.waterConsumedLitres;
  });

  const datasets = Object.keys(yearMap).sort().map((year, idx) => ({
    label: year,
    data: yearMap[year],
    backgroundColor: `hsl(${200 + idx * 30}, 80%, 60%)`, // More blue hues
  }));
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: { display: true },
      title: { display: true, text: 'Water Consumption by Month (Segmented by Year)' },
    },
    scales: {
      x: {
